...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
    ...
    if (((10.0 * root->bar) + root->index) == ((10.0 * bar) + index)) {
        return root;
    } else if (((10.0 * root->bar) + root->index) > ((10.0 * bar) + index)){
...
BST_Node *reverse_tree(BST_Node *root, int min, int max) {
    // Reverse a BST so that the nodes are ordered from ascending to descending, with the bars and index reversed as well.
    if (root == NULL) {
        return NULL;
    }
    
    int diff = root->bar - min;
    ...
    root->left = reverse_tree(root->left, min, max);
    root->right = reverse_tree(root->right, min, max);
    return root;
}
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    root = reverse_tree(root, MIN_BAR, MAX_BAR);
    return root;  
...
double freqfind(int semitones, double freq) {
    // Finding the new frequency and checking to see if the new frequency is valid
    for (int i = 0; i < 100; i++) {
        if ((note_freq[i] == freq) && (0 <= i + semitones) && (i + semitones < 100)) {
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    newfreq = freqfind(semitones, root->freq);

    if ((newfreq != -1) && (time_shift + root->index >= 0) && (time_shift + root->index < 1)) {
